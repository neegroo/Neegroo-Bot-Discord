  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **Necesitas estar en un canal de voz para poder escuchar musica!**"
      );
    if (
      ...
      return client.sendTime(
        message.channel,
        "❌ | **Necesitas estar en el canal de voz para poder usar ese comando!**"
      );
    let SearchString = args.join(" ");
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Uso - **\`${GuildDB.prefix}play [song]\``
      );
    ...
    if (!player)
      return client.sendTime(
        message.channel,
        "❌ | **No hay nada sonando ahora mismo...**"
      );
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          return client.sendTime(
            message.channel,
            "**Ningun resultado encontrado para - **" + SearchString
          );
        }
      } else {
        ...
        if (!player)
          return client.sendTime(
            message.channel,
            "❌ | **Nada esta sonando ahora mismo...**"
          );

        if (Searched.loadType === "NO_MATCHES")
          return client.sendTime(
            message.channel,
            "**Ningun resultado para - **" + SearchString
          );
          ...
          if (!player.playing && !player.paused && !player.queue.size)
            player.play();
          SongAddedEmbed.setAuthor(`Añadida a la cola`, client.botconfig.IconURL);
